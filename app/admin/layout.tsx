...
import { LogOut, LayoutDashboard, Map, Mail, MapPin, Send, Car, Route, CalendarDays, Shapes, Images, Star } from "lucide-react";
...
const navItems = [
  ...
  { href: "/admin/trip-requests", label: "Trip Requests", icon: Route },
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Not signed in → the only reachable admin page is /admin/login
  // (middleware redirects everything else), so render it without the
  // dashboard chrome.
  if (!user) {
      ...
      <div className="flex-1 min-w-0">
        <header className="md:hidden bg-navy text-white flex items-center justify-between px-5 h-16">
          <Link href="/admin" className="flex items-center gap-2.5">
            <span className="relative w-8 h-8 rounded-full bg-white flex items-center justify-center overflow-hidden shrink-0">
              <Image src="/images/logo.jpg" alt="" width={28} height={28} className="object-contain w-7 h-7" />
            </span>
            <span className="font-display text-lg">Admin</span>
          </Link>
          {user && (
