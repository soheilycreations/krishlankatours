...
} from "@/components/GoogleTranslate";
...
export default function LanguageDropdown({
  ...
  useEffect(() => {
    ...
    setActiveCode(gtLang && gtLang !== "en" ? gtLang : locale);
  }, [locale]);
  ...
  const handleSelect = (lang: LangOption) => {
    setOpen(false);
    if (lang.native) {
      // wipe any Google-translated state, then hard-navigate to the
      // locale route so the DOM is rebuilt in the original language
      clearGoogleTranslate();
      const path = pathname === "/" ? "" : pathname;
      window.location.assign(`/${lang.code}${path}`);
    } else {
